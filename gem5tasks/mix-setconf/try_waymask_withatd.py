...
parser.add_argument('--insts-stop-event-afterwarm',type=int,default=5_000_000)
parser.add_argument('--insts-test-afterstop',type=int,default=5_000_000)
args = parser.parse_args()

def find_waymask_mspec(workloads_dict, run_once_script, out_dir_path,
	insts_afterwarm,insts_stop_event_afterwarm,insts_test_afterstop,
 threads=1, ncores=128):
	...
	for w in workloads_dict:
		...
		d_atdpart = d_waypart.copy()
		d_atdpart['l3_qos_policy_set'] = 'BaseAtdPolicy'
		# d_atdpart['l3_qos_policy_set'] = 'FullAtdPolicy'
		d_atdpart["insts-stop-event"] = insts_stop_event_afterwarm
		# d_atdpart["insts-test-afterstop"] = insts_test_afterstop
		# a.append(d_atdpart)

		# d_hpatdpart = d_atdpart.copy()
		# d_hpatdpart['l3_qos_policy_set'] = 'HitPreventAtdPolicy'
		# a.append(d_hpatdpart)
		# d_fmatdpart = d_atdpart.copy()
		# d_fmatdpart['l3_qos_policy_set'] = 'FreeMaskAtdPolicy'
		# a.append(d_fmatdpart)
		# d_signpart = d_atdpart.copy()
		# d_signpart['l3_qos_policy_set'] = 'SignBasePolicy'
		# a.append(d_signpart)
		# d_gtsignpart = d_atdpart.copy()
		# d_gtsignpart['l3_qos_policy_set'] = 'GrowTrainSignPolicy'
		# a.append(d_gtsignpart)
		
		# d_aimd_lowpart = d_atdpart.copy()
		# d_aimd_lowpart['l3_qos_policy_set'] = 'AIMDBasePolicy'
		# d_aimd_lowpart['l3_qos_policy_set'] = 'FullGrowAIMDPolicy'
		# d_aimd_lowpart['l3_qos_policy_set'] = 'OneLessAIMDPolicy'
		# a.append(d_aimd_lowpart)

		# d_sign_withtestpart = d_signpart.copy()
		# d_sign_withtestpart["insts-test-afterstop"] = insts_test_afterstop
		# a.append(d_sign_withtestpart)
		# d_gtsign_withtestpart = d_gtsignpart.copy()
		# d_gtsign_withtestpart["insts-test-afterstop"] = insts_test_afterstop
		# a.append(d_gtsign_withtestpart)
				...
				if len(pending_proc) < max_pending_proc:
					...
					if highways > 0:
						...
						if 'l3_qos_policy_set' in workload:
							...
							addition_cmd.append(f"--insts-stop-event-afterwarm={workload['insts-stop-event']}")
							highways += f"{workload['insts-stop-event']//1000000}MTrain"
							if 'insts-test-afterstop' in workload:
								addition_cmd.append(f"--insts-test-afterstop={workload['insts-test-afterstop']}")
								highways += f"{workload['insts-test-afterstop']//1000000}MTest"
					if highways == 0:
						highways = 'nopart'
...
if __name__ == '__main__':
	...
	insts_stop_event_afterwarm = args.insts_stop_event_afterwarm
	insts_test_afterstop = args.insts_test_afterstop
	...
	insts_stop_event_afterwarm=insts_stop_event_afterwarm,
	insts_test_afterstop=insts_test_afterstop,
	ncores = args.ncores)
