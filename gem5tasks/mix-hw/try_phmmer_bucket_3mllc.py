...
def find_tb_mspec(workloads, run_once_script, out_dir_path, threads=1, ncores=128):
	...
	base_arguments.append('--l3_assoc=12')
			...
			for workload in workloads[:can_launch]:
				if len(pending_proc) < max_pending_proc:
					...
					if len(waymask_set)>0:
						...
						waymask_set = 'nopart'
					l3_tb_freq = workload.get("--l3_tb_freq","1024")
					addition_cmd.append(f"--l3_tb_freq={l3_tb_freq}")
					l3_tb_inc = workload.get("--l3_tb_inc","1024")
					addition_cmd.append(f"--l3_tb_inc={l3_tb_inc}")
					result_path = os.path.join(out_dir_path, f"{workload_name}/l3-{waymask_set}/l2-nopart/l3-tb-{l3_tb_freq}-{l3_tb_inc}")
...
if __name__ == '__main__':
	log_dir = f"/nfs/home/zhangchuanqi/lvna/5g/ff-reshape/log/new_hw_test/period_hmmer_o3_0-period_hmmer_o3_3-period_hmmer_o2_0-period_hmmer_o2_2/3072kBLLC/try-tb/"
