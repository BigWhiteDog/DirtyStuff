...
parser.add_argument("--after-warmM", type=int, default=16)
parser.add_argument('-W','--warmup',type=int,default=20_000_000)
parser.add_argument('--notie',action='store_true',default=False)
args = parser.parse_args()

def find_waymask_mspec(workloads, run_once_script, out_dir_path, warmup=20_000_000,
	after_warmM=None, threads=1, ncores=128):
	base_arguments = ["python3", run_once_script, "--cpt-json", json_path, '-W', str(warmup), '--np=4'
	,'--cycle_per_tti',str(4_000_000)]
	if after_warmM:
			...
			for workload in workloads[:can_launch]:
				if len(pending_proc) < max_pending_proc:
					...
					if len(waymask_set)>0:
						...
						waymask_set = 'nopart'
					result_path = os.path.join(out_dir_path, f"{workload_name}/l3-{waymask_set}/l2-nopart")
...
if __name__ == '__main__':
	if args.after_warmM:
		log_dir = f"/nfs/home/zhangchuanqi/lvna/5g/ff-reshape/log/new_hw_test/{args.after_warmM}M/"
	else:
		log_dir = f"/nfs/home/zhangchuanqi/lvna/5g/ff-reshape/log/new_hw_test/full{args.warmup}/"

	target_workload = ['hmmer_o31-hmmer0-hmmer_o30-hmmer1']
